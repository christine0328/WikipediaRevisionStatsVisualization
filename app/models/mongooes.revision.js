...
var nonMinorRevisionsPipeline = [
		{'$match':{minor:{'$exists':false}, title:"CNN"}},
		{'$group':{'_id':'$user', 'numOfEdits': {$sum:1}}},
		{'$sort':{numOfEdits:-1}},
		{'$limit':1}
]
Revision.aggregate(nonMinorRevisionsPipeline, function(err, results){
	if (err){
		console.log("Aggregation Error")
	}else{
		console.log("Non-minor Revisions on CNN is: ")
		console.log(results)
	}
})


//find the number of revisions on article "CNN"
Revision.find({'title':'CNN'})
.count()
.exec(function(err,result){
	if (err){
		...
		console.log("Total number of revisions on CNN is: ");
		console.log(result)
	}	
})